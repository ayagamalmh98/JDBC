...
public class ValidReadQuery implements Parser {
    ...
    private int selectIsValid(String query) {
        return Math.max(Math.max(regexMatcher(query, selectAllPattern) ? 15 : -1, regexMatcher(query, selectAllWherePattern) ? 6 : -1),Math.max(Math.max(regexMatcher(query, selectSomePattern) ? 7 : -1, regexMatcher(query, selectSomeWherePattern) ? 8 : -1),regexMatcher(query,selectAsPattern)?5:-1));
    }
